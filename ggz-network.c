...
static int get_players(void)
{
  ...
  for (i = 0; i < num_players; i++) {
	...
	g_free(tmp);
      ...
      if (i == 0) {
        tmp = g_strdup_printf(_("The game ended because the host %s left the game.\n"), 
				names[i]);
        add_chat_text(tmp);
        g_free(tmp);
      }
...
static gboolean handle_ggzmod(GIOChannel * channel, GIOCondition cond,
			      gpointer data)
{
  GGZMod *mod = data;
...
void network_game_move(guint x)
{
