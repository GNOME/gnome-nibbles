...
{
    const double PIx2 = 6.28318530717958647692528676655900577; /* Pi times 2 */
    /* colour property */
...
internal class ColourWheel : Widget
{
    ...
    public override void snapshot (Snapshot snapshot)
    {
        ...
        draw_label (snapshot.append_cairo ({{0 , 0}, {get_width (), get_height ()}}), get_width (), get_height (),
    ...
    void draw_label (Cairo.Context c, double width, double height, string text)
    {
        ...
        double text_width;
        double text_height;
        int font_size = calculate_font_size (c, text, (int)width - border, out text_width, out text_height);
